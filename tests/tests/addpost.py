...
POST_BODY = ("This is a test post. "
    "There is no reason you should be paying attention to it. {}".format(POST_RANDOM))
...
def test(driver, username, password):
    ...
    assert driver.title == ADD_POST_TITLE, "Title is {}".format(driver.title)
    ...
    assert post != None
    assert post.title == POST_TITLE, "Title is {}".format(post.title)
    assert POST_RANDOM in post.body
    assert tags == POST_TAGS, "Tags are {}".format(tags)
